...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($titre) || empty($contenu) || empty($categorie_id)) {
        ...
        $query = $pdo->prepare("SELECT COUNT(*) FROM histoires WHERE titre = ?");
        $query->execute([$titre]);
        $existingTitle = $query->fetchColumn();

        if ($existingTitle > 0) {
            $error_message = "Le titre existe déjà dans la base de données.";
        } else {
            // Insertion dans la base de données
            $query = $pdo->prepare("INSERT INTO histoires (titre, contenu, categorie_id) VALUES (?, ?, ?)");
            $query->execute([$titre, $contenu, $categorie_id]);
...
$query = $pdo->query("SELECT id, nom FROM categorie");
              ...
              <div class="col-md-12">
                <div class="card">
                  <div class="card-header">
                    <div class="card-title">Ajouter une histoire</div>
                  </div>
                    <div class="card-body">
                      <div class="row">
                      <form id="histoireForm" method="POST" action="">
                        <label for="titre">Titre</label><br>
                        <input class="form-control form-control-user" type="text" id="titre" name="titre" required value="<?php echo htmlspecialchars($titre ?? ''); ?>"><br>

                        <label for="contenu">Contenu</label><br>
                        <textarea class="form-control form-control-user" id="contenu" name="contenu" required><?php echo htmlspecialchars($contenu ?? ''); ?></textarea><br>
                        ...
                        <select class="form-control form-control-user" id="categorie_id" name="categorie_id" required>
                            ...
                            foreach ($categories as $categorie) {
                                // Ajout de la sélection si la catégorie est déjà choisie
                                $selected = isset($categorie_id) && $categorie_id == $categorie['id'] ? 'selected' : '';
