...
/* Make the table container larger */
.container.centered-form {
    max-width: 95%; /* 95% of the page width */
    padding: 20px;
    margin-top: 20px;
}
...
/* Optional: Increase table row height for better visibility */
.table th, .table td {
    padding: 12px;
    text-align: center;
}

/* Optional: Set a fixed height for modals to make them consistent */
.modal-dialog {
    ...
    <div class="wrapper">
      ...
      <div class="main-panel">
        <div class="main-header">
          ...
          <div class="container centered-form" style="max-width: 90%; padding: 20px;">
    <!-- Ingredient Table Card -->
            <h4 class="card-title mb-4 text-center">Ingredient List</h4>
            <?php
                require_once "../../Controller/connexion.php"; // Ensure the correct path

                try {
                    // Fetch all ingredients from the database
                    $sql = "SELECT * FROM ingredient";  // Modify the table name if needed
                    ...
                    if ($stmt->rowCount() > 0) {
                        ...
                        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                            $id_ingredient = $row['ID_ingredient'] ?? 'N/A'; 
                            $nom = $row['nom'] ?? 'No Name'; 
                            $unite = $row['unite'] ?? 'No Unit';
                            $categorie = $row['Categorie'] ?? 'N/A';

                            echo '<tr>';
                            echo '<td>' . htmlspecialchars($id_ingredient) . '</td>';
                            echo '<td>' . htmlspecialchars($nom) . '</td>';
                            echo '<td>' . htmlspecialchars($unite) . '</td>';
